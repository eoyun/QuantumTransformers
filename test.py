...
train_transforms = transforms.Compose([
    ...
    transforms.RandomApply([
        transforms.RandomChoice([
            transforms.RandomAffine(degrees=0, scale=(0.6, 0.9), fill=0),   # zoom out (scale down to 60-90%)
            transforms.RandomAffine(degrees=0, scale=(1.1, 1.4), fill=0)    # zoom in (scale up to 110-140%)
        ])
    ], p=0.5),
    # Random Rotation (approximately ±72° max)
    ...
    # Random Shear (use RandomAffine for shear only)
    transforms.RandomApply([transforms.RandomAffine(degrees=0, shear=20, fill=0)], p=0.5),  # ~20 degrees shear
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
])

# Transforms for validation/test (no augmentation, just resize + normalize)
...
def focal_loss(inputs, targets, alpha=0.25, gamma=2.0, reduction='mean'):
    ...
    p_t = torch.exp(-ce_loss)  # this is equivalent to p_t = softmax(inputs)[range(n), targets]
...
# Instantiate optimizer (AdamW with lr=1e-5)
